...
class SelectNumberVC: UIViewController, UITextFieldDelegate, PhoneNumberProtocol{
    
    var controller: PhoneNumberController?
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        controller = PhoneNumberController(view: self)
        texFieldSetup()
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(selectNumber.text == nil || selectNumber.text!.isEmpty) {
            onError(message: "Номер не введен", code: 0)
        } else {
            self.controller?.onEnter(phoneNumber: selectNumber.text!)
        }
